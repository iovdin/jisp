(function() {
  ...
  function compileForm(form, scope, opts, nested) {
    ...
    try {
      ...
      if (!e.src) e.src = form;
      if ((!e.context && (JSON.stringify(form).length > 150))) e.context = form;
      throw e;
    }
    return _ref16;
  }
  ...
  function compile(src, opts) {
    var defaults, parsed, compiled, scope, startErr, endErr, res, start, end, _ref, _i, _ref0, _ref1, _i0;
    try {
      defaults = {
        wrap: true,
        topScope: true,
      ...
      compiled = _ref[0];
      scope = _ref[1];
      _ref0 = (opts.pretty && (typeof beautify !== 'undefined')) ? beautify(render(compiled), {
        indent_size: 2
      }) : render(compiled);
    } catch (e) {
      startErr = "\x1b[41m";
      ...
      function form2str(form, tab) {
        var showErr, item, key, value, _i0, _res, _ref1, _len, _ref2, _ref3, _ref4, _res0, _ref5, _len0, _ref6;
        if ((typeof tab === 'undefined')) tab = "";
        if (Array.isArray(form)) {
          showErr = JSON.stringify(form) === JSON.stringify(e.src);
          if (((form[0] === "get") && (typeof form[2] === "string") && (form[2][0] === "."))) {
            _ref3 = (form2str(form[1]) + form[2]);
          } else {
            _res = [];
            _ref1 = form;
            for (_i0 = 0, _len = _ref1.length; _i0 < _len; ++_i0) {
              item = _ref1[_i0];
              if (typeof(_ref2 = form2str(item, tab + "  ")) !== 'undefined') _res.push(_ref2);
            }
            _ref3 = ((tab ? ("\n" + tab) : "") + "(" + _res.join(" ") + ")");
          }
          _ref4 = ((showErr ? startErr : "") + _ref3 + (showErr ? endErr : ""));
        } else if (typeof form === "object") {
          _res0 = [];
          _ref5 = form;
          for (key in _ref5) {
            value = _ref5[key];
            if (typeof(_ref6 = ("\"" + key + "\" " + form2str(value))) !== 'undefined') _res0.push(_ref6);
          }
          _ref4 = ("(: " + _res0.join("  ") + ")");
      ...
      form2str;
      res = form2str(e.context || e.src);
      _ref1 = res
        .split("\n")
        .reduce((function(memo, line, index) {
          if ((-1 !== line.indexOf(startErr))) memo[0] = index;
          if ((-1 !== line.indexOf(endErr))) memo[1] = index;
          return memo;
        }), []);
      start = _ref1[0];
      end = _ref1[1];
      console.error(res
        ...
        .filter((function(line, index) {
          return ((index < (end + 7)) && (index > (start - 7)));
        }))
        .join("\n"));
      delete e.context;
      delete e.src;
